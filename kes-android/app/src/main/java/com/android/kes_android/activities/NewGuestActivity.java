...
public class NewGuestActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView mName = (TextView) findViewById(R.id.newguest_name);

        mImage = (ImageView) findViewById(R.id.newguest_image);
        Button mImageButton = (Button) findViewById(R.id.newguest_addphoto_button);
        Button mSubmit = (Button) findViewById(R.id.newguest_submit);
        ...
        mSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (fullName.isEmpty() || mSelectedImagePath.isEmpty()) {
                    Toast.makeText(mContext, "Please complete all fields", Toast.LENGTH_SHORT).show();
                } else {
                    File f = new File(mSelectedImagePath);
                    createNewUser(fullName, f);
                }
            }
        });
    }

    private void createNewUser(String fullName, File photo) {
        ...
        try {
            params.put("guest_photo", photo);
        } catch (Exception e) {
            e.printStackTrace();
        }

        client.post(API.ADD_GUEST, params, new JsonHttpResponseHandler() {
            ProgressDialog mProgressDialog;

            @Override
            public void onStart() {
                mProgressDialog = new ProgressDialog(mContext, ProgressDialog.THEME_HOLO_LIGHT);
                mProgressDialog.setTitle("Creating Guest");
            ...
            public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
                Toast.makeText(mContext, "Creating Guest Failed", Toast.LENGTH_SHORT).show();
            }
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    if (!API.isSuccess(response.getString("result"))) {
                        ...
                        Toast.makeText(mContext, "Creating Guest Complete", Toast.LENGTH_SHORT).show();
                    }
